...
func Test_certificatesUseCase_IssueCertificate(t *testing.T) {
	...
	tests := []struct {
		...
		{"failure(GetVaultVersionIfExists)", &mock.VaultRepository{GetVaultVersionDataIfExistsBool: true, GetVaultVersionDataIfExistsResource: "test", GetVaultVersionDataIfExistsData: []byte("test"), GetVaultVersionIfExistsBool: false, GetVaultVersionIfExistsErr: fixture.ErrTestError}, nil, args{contexts.WithLogger(context.TODO(), rec.Must(rec.New(io.Discard))), true, "test", "test", 30, []string{"localhost"}}, "", "", true},
	}
	for _, tt := range tests {
		tt := tt
		t.Run(tt.name, func(t *testing.T) {
...
func Test_certificatesUseCase_issueCertificate(t *testing.T) {
	...
	tests := []struct {
		...
		{"success(RenewPrivateKey)", &mock.VaultRepository{GetVaultVersionDataIfExistsBool: true, GetVaultVersionDataIfExistsResource: "test", GetVaultVersionDataIfExistsData: []byte("test"), GetVaultVersionIfExistsBool: true, GetVaultVersionIfExistsVersion: "test", AddVaultVersionResource: "test"}, &mock.LetsEncryptRepository{}, args{contexts.WithLogger(context.TODO(), rec.Must(rec.New(io.Discard))), true, "test", "test", 30, []string{"localhost"}, nil}, "test", "test", false},
		...
		{"failure(GetVaultVersionIfExists)", &mock.VaultRepository{GetVaultVersionIfExistsErr: fixture.ErrTestError}, &mock.LetsEncryptRepository{}, args{contexts.WithLogger(context.TODO(), rec.Must(rec.New(io.Discard))), false, "test", "test", 30, []string{"localhost"}, func(pemData []byte) (notyet bool, daysToStart int64, expired bool, daysToExpire int64, err error) {
			return false, -1, false, 80, nil
		}}, "", "", true},
		{"failure(GetVaultVersionDataIfExists)", &mock.VaultRepository{GetVaultVersionDataIfExistsBool: true, GetVaultVersionDataIfExistsResource: "test", GetVaultVersionDataIfExistsErr: fixture.ErrTestError}, &mock.LetsEncryptRepository{}, args{contexts.WithLogger(context.TODO(), rec.Must(rec.New(io.Discard))), false, "test", "test", 30, []string{"localhost"}, func(pemData []byte) (notyet bool, daysToStart int64, expired bool, daysToExpire int64, err error) {
			return false, -1, false, 80, nil
		}}, "", "", true},
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			gotPrivateKeyVaultVersionResource, gotCertificateVaultVersionResource, err := s.issueCertificate(tt.args.ctx, tt.args.privateKeyRenewed, tt.args.privateKeyVaultResource, tt.args.certificateVaultResource, tt.args.thresholdOfDaysToExpire, tt.args.domains, tt.args.checkCertificatePEMFunc)
			if (err != nil) != tt.wantErr {
				t.Errorf("certificatesUseCase.issueCertificate() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
